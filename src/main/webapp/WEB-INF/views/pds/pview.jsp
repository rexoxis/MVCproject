        ...
        <div class="row" style="margin: 10px 30px 20px 50px">
            <table class="table">
                ...
                <tr style="background-color: #ccff99">
                    <td>${p.pwriter}</td>
                    <td class="text-right">${fn:substring(p.pregdate,0,10)} / ${p.pviews}</td></tr>

                <tr style="background-color: #ffffcc">
                    <td colspan="2" style="border-bottom: 2px solid #000000o">
                        <%--줄바꿈을 인식하게 하기위해 \n을 br태그로 바꿈--%>
                            ${fn:replace(p.pcontents, newchar, "<br>") }
                    </td>
                </tr>
                <tr style="background-color: #ffffcc">
                    <td colspan="2">
                        ...
                        <%--업로드한 이미지 표시--%>
                        <img src="/pds/showimg?f=${p.file1}" width="20%"><br>
                        <span style="font-size: 12px">첨부파일1 : <a href="download?f=${p.file1}"> ${p.file1} </a>
                        (다운획수 : 123, 크기 : 232MB)</span>
                        <%--다운로드--%>
                        <p><a href="download?f=${p.file1}">다운로드</a></p>
...
<script>
    ...
    $(function() {
        $('#newbtn').on('click',function(e) {location.href='/pds/pwrite';});
    });
    $(function() {
        $('#listbtn').on('click',function(e) {location.href='/pds/plist';});
    });
    $(function() {
        $('#updatebtn').on('click',function(e) {location.href='/pds/pupdate';});
    });
