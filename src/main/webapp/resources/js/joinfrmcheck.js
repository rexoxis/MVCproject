...
var frm = document.joinmemberfrm;

var juimin1 = frm.jumin1.value;
var juimin2 = frm.jumin2.value;
...
function joinfrmcheck() {
    ...
    var zipcode = frm.zip1.value + frm.zip2.value;
    ...
    } else if(!jumincheck(juimin1, juimin2)){
        alert("올바른 번호가 아닙니다");

    } if (!joinuidrex.test(frm.userid.value)){
        alert("아이디를 확인해주세요.");

    } else if (!pwdrex.test(frm.joinpwd.value)){
        alert("비밀번호는 숫자,특수문자, 영문대소문자로만 가능합니다.");

    } else if (frm.joinpwd.value < 6 || frm.joinpwd.value >= 16){
        alert("비밀번호는 6자이상 16자 이하입니다.");

    } else if (frm.chjoinpwd.value != frm.joinpwd.value){
        alert("비밀번호가 일치하지 않습니다.");

    } else if (isNaN(zipcode)){
        alert("우편번호는 숫자로만 입력 가능합니다.");
